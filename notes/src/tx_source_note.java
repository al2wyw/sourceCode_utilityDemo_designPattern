...
TransactionSynchronizationManager(thread oriented):
...
DataSourceTransactionObject of DataSourceTransactionManager hold the connectionholder, savePointAllow(b), newConnectionHolder(b), mustRestoreAutoCommit(b)
...
Transaction(DataSourceTransactionObject) and TransactionSynchronization two main things go along with the transactional
...
MapperProxy invoke -> MapperMethod execute -> sqlSessionTemplate insert/delete/update/select -> sqlSessionProxy invoke{
	...
	try{
		...
		//(holder != null) && (holder.getSqlSession() == session)

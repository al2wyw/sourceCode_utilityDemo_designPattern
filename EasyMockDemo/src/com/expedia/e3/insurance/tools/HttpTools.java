...
public class HttpTools
{
	...
	{	
		...
		{										
			...
			{ 
				HttpsURLConnection connectS = (HttpsURLConnection) url.openConnection();				
			...
			con.setReadTimeout( readTimeout > 0 ? readTimeout : DEFAULT_READ_TIMEOUT );
			con.setConnectTimeout(connectTimeout > 0 ? connectTimeout : DEFAULT_CONNECT_TIMEOUT);
	...
	static public String doPost(URLConnection con, String requestString) throws IOException
	{
		...
		if (con != null)
		{
			try
			{
				DataOutputStream requestWriteStream = new DataOutputStream(con.getOutputStream());

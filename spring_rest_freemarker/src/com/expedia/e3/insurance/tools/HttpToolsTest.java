...
public class HttpToolsTest {
	...
	@Test
	public void testPostRequestByGlobalURLConnectionWithHttp()
			throws IOException {
...
//		 EasyMock.replay();
		...
		String result = HttpTools.postRequestByGlobalURLConnection(
				"http://localhost:8088", "request=help", "GET", -1, -1);
		Assert.assertNotNull(result);
		Assert.assertEquals(expected, result);
	}

	// @Test
	// public void testPostRequestByGlobalURLConnectionWithHttps() throws
	// IOException{
	// URL url = EasyMock.createMock(URL.class);
	// HttpsURLConnection httpsCon =
	// EasyMock.createMock(HttpsURLConnection.class);
	// EasyMock.expect(url.openConnection()).andReturn(httpsCon).anyTimes();
	// EasyMock.replay();
	// String expected = "test1test2test3";
	// String result =
	// HttpTools.postRequestByGlobalURLConnection("https://testforfun.com",
	// "request=help", "get", 45, 43);
	// Assert.assertNotNull(result);
	// Assert.assertEquals(expected,result);
	// }
}